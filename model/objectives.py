...
def compute_sdm(
    ...
    if use_sigmoid:
        # Sigmoid-based probabilities with row-wise normalization for a valid distribution
        t2i_prob = torch.sigmoid(text_proj_image)
        t2i_prob = t2i_prob / (t2i_prob.sum(dim=1, keepdim=True) + epsilon)

        # Mirror i2t path for symmetry
        i2t_cosine_theta = t2i_cosine_theta.t()
        image_proj_text = logit_scale * i2t_cosine_theta + logit_bias
        i2t_prob = torch.sigmoid(image_proj_text)
        i2t_prob = i2t_prob / (i2t_prob.sum(dim=1, keepdim=True) + epsilon)

        # KL(pred || labels) for both directions to match softmax branch behavior
        t2i_loss = t2i_prob * (
            torch.log(t2i_prob + epsilon) - torch.log(labels_distribute + epsilon)
        )
        i2t_loss = i2t_prob * (
            torch.log(i2t_prob + epsilon) - torch.log(labels_distribute + epsilon)
        )
...
def compute_ritc(
    ...
    if use_sigmoid:
        # Map targets from [-1, 1] to [0, 1] and normalize rows
        sim_targets = (sim_targets + 1) / 2
        sim_targets = sim_targets / (sim_targets.sum(dim=1, keepdim=True) + eps)

        # Predicted probabilities via sigmoid, normalized to distributions
        prob_i2t = torch.sigmoid(sim_i2t)
        prob_i2t = prob_i2t / (prob_i2t.sum(dim=1, keepdim=True) + eps)
        prob_t2i = torch.sigmoid(sim_t2i)
        prob_t2i = prob_t2i / (prob_t2i.sum(dim=1, keepdim=True) + eps)

        # Use KL(target || pred) consistent with softmax branch orientation
        target_log = (sim_targets + eps).log()
        kl_img = F.kl_div(target_log, (prob_i2t + eps).log(), log_target=True, reduction="batchmean")
        kl_txt = F.kl_div(target_log, (prob_t2i + eps).log(), log_target=True, reduction="batchmean")
